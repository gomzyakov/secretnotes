...
namespace App\Services\LaravelLocalization\Middleware;

use App\Services\LaravelLocalization\LanguageNegotiator;
use Closure;
...
class LocaleSessionRedirect extends LaravelLocalizationMiddlewareBase
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        if (
            $locale && app('laravellocalization')->checkLocaleInSupportedLocales($locale) && ! (app('laravellocalization')->isHiddenDefault($locale))
        ) {
            app('session')->reflash();
